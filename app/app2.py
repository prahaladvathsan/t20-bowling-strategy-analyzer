...
with st.sidebar:
    ...
    # Add an info section about Smart Stats if available
    if SMART_STATS_AVAILABLE:
...
if st.session_state.current_page == "Batter Analysis":
    ...
    if selected_batter:
        ...
        with st.spinner("Analyzing batter..."):
            ...
            if batter_profile:
                ...
                # Create tabs for different analysis views - add Smart Stats if available
                tab_titles = ["Vulnerability Heatmap", "Performance by Bowler Type",
                            "Performance by Phase", "Performance by Bowling Style"]
                
                if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None:
                    tab_titles.append("Smart Stats")
                ...
                # Add Smart Stats tab if available
                if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None and len(tabs) >= 5:
                    with tabs[4]:
                        st.subheader("Smart Stats Analysis")
                        
                        try:
                            # Get batsman's smart stats
                            batsman_stats = st.session_state.smart_analyzer.get_batsman_smart_stats(batsman_name=selected_batter)
                            ...
                            if batsman_stats and batsman_stats['matches'] > 0:
                                ...
                                with col1:
                                    st.metric("Smart Runs", f"{batsman_stats['smart_runs_total']:.2f}")
                                with col2:
                                    # Show delta between smart and conventional
                                    delta = batsman_stats['smart_sr_overall'] - batsman_stats['conventional_sr_overall']
                                    st.metric("Smart Strike Rate", 
                                             f"{batsman_stats['smart_sr_overall']:.2f}", 
                                             delta=f"{delta:+.2f} vs Conv.")
                                with col3:
                                    st.metric("Impact Score", f"{batsman_stats['avg_impact_per_match']:.2f}")
                                
                                # Create visualization
                                st.subheader("Smart vs Conventional Performance")
                                ...
                                if len(performances_df) > 0:
                                    ...
                                    x = np.arange(len(performances_df))
                                    width = 0.35
                                    
                                    ax.bar(x - width/2, performances_df['conventional_runs'], width, 
                                          label='Conventional Runs', color='skyblue')
                                    ax.bar(x + width/2, performances_df['smart_runs'], width, 
                                          label='Smart Runs', color='orange')
                                    
                                    # Add labels
                                    ax.set_xlabel('Match Number')
                                    ax.set_ylabel('Runs')
                                    ax.set_title(f'{selected_batter} - Smart vs Conventional Runs')
                                    ax.set_xticks(x)
                                    ax.set_xticklabels(performances_df['match_num'])
                                    ax.legend()
                                    ...
                                    # Show the match data
                                    st.dataframe(
                                        performances_df[['match_id', 'conventional_runs', 'smart_runs', 
                                                       'conventional_sr', 'smart_sr', 'impact']],
                                        column_config={
                                            'match_id': 'Match',
                                            'conventional_runs': 'Conv. Runs',
                                ...
                                st.info("No Smart Stats data available for this batter.")
                        except Exception as e:
                            st.info(f"Smart Stats analysis not available: {e}")
                            st.info("Please run the backend processor with Smart Stats enabled.")
    ...
    if selected_bowler:
        ...
        if bowler_profile:
            ...
            # Create tabs - add Smart Stats if available
            tab_titles = ["Economy Rate", "Strike Rate"]
            ...
            with tabs[0]:
                st.subheader("Line & Length Effectiveness - Economy")
            ...
            with tabs[1]:
                st.subheader("Line & Length Effectiveness - Strike Rate")
                line_length_stats = bowler_profile.get('by_line_length', {})
                if line_length_stats:
            ...
            # Add Smart Stats tab if available
            if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None and len(tabs) >= 3:
                with tabs[2]:
                    st.subheader("Smart Stats Analysis")
                    
                    try:
                        # Get bowler's smart stats
                        bowler_stats = st.session_state.smart_analyzer.get_bowler_smart_stats(bowler_name=selected_bowler)
                        ...
                        if bowler_stats and bowler_stats['matches'] > 0:
                            ...
                            with col1:
                                st.metric("Smart Wickets", f"{bowler_stats['smart_wickets_total']:.2f}")
                            with col2:
                                # Show delta between smart and conventional
                                delta = bowler_stats['conventional_er_overall'] - bowler_stats['smart_er_overall']
                                st.metric("Smart Economy Rate", 
                                         f"{bowler_stats['smart_er_overall']:.2f}", 
                                         delta=f"{delta:+.2f} vs Conv.")
                            with col3:
                                st.metric("Impact Score", f"{bowler_stats['avg_impact_per_match']:.2f}")
                            ...
                            if len(performances_df) > 0:
                                ...
                                ax.bar(x - width/2, performances_df['conventional_wickets'], width, 
                                      label='Conventional Wickets', color='skyblue')
                                ax.bar(x + width/2, performances_df['smart_wickets'], width, 
                                      label='Smart Wickets', color='orange')
                                
                                # Add labels
                                ax.set_xlabel('Match Number')
                                ax.set_ylabel('Wickets')
                                ax.set_title(f'{selected_bowler} - Smart vs Conventional Wickets')
                                ax.set_xticks(x)
                                ax.set_xticklabels(performances_df['match_num'])
                                ax.legend()
                                ...
                                # Show the match data
                                st.dataframe(
                                    performances_df[['match_id', 'conventional_wickets', 'smart_wickets', 
                                                   'conventional_er', 'smart_er', 'impact']],
                                    column_config={
                                        'match_id': 'Match',
                                        'conventional_wickets': 'Conv. Wickets',
                                        'smart_wickets': 'Smart Wickets',
                                        'conventional_er': 'Conv. ER',
                                        'smart_er': 'Smart ER',
                                        'impact': 'Impact'
                                    }
                                )
                        else:
                            st.info("No Smart Stats data available for this bowler.")
                    except Exception as e:
                        st.info(f"Smart Stats analysis not available: {e}")
                        st.info("Please run the backend processor with Smart Stats enabled.")
    ...
    with col2:
        ...
        all_bowl_types = st.session_state.bowler_analyzer.get_all_bowler_types() if hasattr(st.session_state.bowler_analyzer, 'get_all_bowler_types') else []
    ...
    if selected_batter:
        ...
        available_bowlers = st.session_state.bowler_analyzer.get_filtered_bowlers(selected_types) if hasattr(st.session_state.bowler_analyzer, 'get_filtered_bowlers') else []
        ...
        # Add a Smart Stats insight section if available
        if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None:
            try:
                # Create an expander for Smart Stats insights
                with st.expander("Smart Stats Insights", expanded=False):
                    st.subheader(f"Smart Stats Analysis: {selected_batter}")
                    ...
                    if batsman_stats and batsman_stats['matches'] > 0:
                        ...
                        with col1:
                            st.metric("Smart SR", f"{batsman_stats['smart_sr_overall']:.2f}")
                        with col2:
                            delta = batsman_stats['smart_sr_overall'] - batsman_stats['conventional_sr_overall']
                            st.metric("SR Differential", f"{delta:+.2f}")
                        with col3:
                            st.metric("Impact Score", f"{batsman_stats['avg_impact_per_match']:.2f}")
                        
                        # If we have matchups, analyze with Smart Stats
                        if matchups and matchups.get('matchups'):
                            top_bowlers = [m['bowler'] for m in matchups['matchups'][:3]]
                            
                            # Get Smart Stats for these bowlers
                            bowler_smart_stats = []
                            for bowler in top_bowlers:
                                stats = st.session_state.smart_analyzer.get_bowler_smart_stats(bowler_name=bowler)
                                if stats and stats['matches'] > 0:
                                    bowler_smart_stats.append({
                                        'bowler': bowler,
                                        'smart_wickets': stats['smart_wickets_total'],
                                        'smart_er': stats['smart_er_overall'],
                                        'impact': stats['avg_impact_per_match']
                                    })
                            
                            if bowler_smart_stats:
                                st.subheader("Smart Stats for Top Bowler Matchups")
                                
                                # Create DataFrame for display
                                df = pd.DataFrame(bowler_smart_stats)
                ...
                st.info(f"Smart Stats insights not available: {e}")
    ...
    if selected_batter and hasattr(st.session_state, 'plan_generator') and st.session_state.plan_generator:
        ...
        if plan and not plan.get('error'):
            ...
            with tab1:
                ...
                # Add Smart Stats analysis if available
                if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None:
                    try:
                        smart_stats = st.session_state.smart_analyzer.get_batsman_smart_stats(batsman_name=selected_batter)
                        if smart_stats and smart_stats['matches'] > 0:
                            st.subheader("Smart Stats Analysis")
                            ...
                            with col1:
                                st.metric("Smart Runs Average", f"{smart_stats['smart_runs_total'] / smart_stats['matches']:.2f}")
                            with col2:
                                st.metric("Smart Strike Rate", f"{smart_stats['smart_sr_overall']:.2f}")
                            with col3:
                                st.metric("Overall Impact Score", f"{smart_stats['avg_impact_per_match']:.2f}")
                                
                            # Add situational context based on Smart Stats
                            st.subheader("Situational Insights")
                            
                            # Compare conventional and smart metrics to provide context
                            if smart_stats['smart_sr_overall'] > smart_stats['conventional_sr_overall'] * 1.1:
                                st.info("⚠️ **High-Pressure Performer**: This batter tends to score more effectively under pressure. Consider defensive field placements even in seemingly low-scoring situations.")
                            elif smart_stats['smart_sr_overall'] < smart_stats['conventional_sr_overall'] * 0.9:
                                st.success("💡 **Pressure Vulnerable**: This batter's effectiveness drops under high-pressure situations. Consider aggressive field placements in crucial moments.")
                    except Exception as e:
                        pass  # Skip Smart Stats if not available
            ...
            with tab3:
                ...
                if matchups:
                    ...
                    # Add Smart Stats bowler analysis if available
                    if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None:
                        try:
                            st.subheader("Smart Stats Bowler Analysis")
                            
                            # Get top 3 bowlers
                            top_bowlers = [matchup['bowler'] for matchup in matchups[:3]]
                            
                            # Create columns for each bowler
                            cols = st.columns(len(top_bowlers))
                            
                            # Display stats for each bowler
                            for i, bowler in enumerate(top_bowlers):
                                with cols[i]:
                                    st.subheader(bowler)
                                    
                                    # Get smart stats
                                    bowler_stats = st.session_state.smart_analyzer.get_bowler_smart_stats(bowler_name=bowler)
                                    
                                    if bowler_stats and bowler_stats['matches'] > 0:
                                        st.metric("Smart Wickets", f"{bowler_stats['smart_wickets_total']:.2f}")
                                        st.metric("Smart Economy", f"{bowler_stats['smart_er_overall']:.2f}")
                                        st.metric("Impact Score", f"{bowler_stats['avg_impact_per_match']:.2f}")
                                    else:
                                        st.write("No Smart Stats data available.")
                        except Exception as e:
            ...
            def export_plan_as_html():
                ...
                # Add Smart Stats section if available
                if SMART_STATS_AVAILABLE and st.session_state.smart_analyzer is not None:
                    try:
                        smart_stats = st.session_state.smart_analyzer.get_batsman_smart_stats(batsman_name=selected_batter)
                        if smart_stats and smart_stats['matches'] > 0:
                            html_content += """
                                <div class="section">
                                    <h2>Smart Stats Analysis</h2>
                            """
                            
                            html_content += f"""
                                <p><strong>Smart Strike Rate:</strong> {smart_stats['smart_sr_overall']:.2f}</p>
                                <p><strong>Conventional Strike Rate:</strong> {smart_stats['conventional_sr_overall']:.2f}</p>
                                <p><strong>Impact Score:</strong> {smart_stats['avg_impact_per_match']:.2f}</p>
                                
                                <h3>Match Performances</h3>
                                <table border="1" cellpadding="5" style="border-collapse: collapse; width: 100%;">
                                    <tr style="background-color: #1e4e8c; color: white;">
                                        <th>Match</th>
                                        <th>Smart Runs</th>
                                        <th>Conventional Runs</th>
                                        <th>Smart SR</th>
                                        <th>Impact</th>
                                    </tr>
                            """
                            ...
                            for perf in smart_stats['match_performances']:
                                html_content += f"""
                                    <tr>
                                        <td>{perf['match_id']}</td>
                                        <td>{perf['smart_runs']:.2f}</td>
                                        <td>{perf['conventional_runs']}</td>
                                        <td>{perf['smart_sr']:.2f}</td>
                                        <td>{perf['impact']:.2f}</td>
                                    </tr>
                                """
                        ...
                        # Skip Smart Stats section if not available
                        pass
                
                html_content += f"""
                    <div class="section">
                        <h2>Summary</h2>
                        <pre style="white-space: pre-wrap;">{plan.get('summary', 'No summary available.')}</pre>
                    </div>
...
# Footer
st.markdown("---")
st.markdown("Created with ❤️ using Streamlit • [GitHub Repository](https://github.com/user/t20-bowling-strategy-analyzer)")